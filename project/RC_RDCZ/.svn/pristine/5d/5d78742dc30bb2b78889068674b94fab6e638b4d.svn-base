 ...
 * @author martin
 */
public class TiskObsahovaPredloha extends BussinessFunctionMother implements CnkpMikrofilm {
    private static final String LOCAL_SECURITY = "tisk102";
    private Object can;
    ...
    public WizardMessage runBusinessMethod() throws ValidationException, WizardException, ApplicationErrorException {
        ...
        rwm.addInfo("Tiskový výstup Obsahová předloha pro čan " + can, null, null, null, null, 0);
        rwm.addLine("<script> setTimeout(\"window.close()\", 10 * 1000); </script>");
        rwm.addLine("<SCRIPT language=\"JavaScript\">window.location=\"" + CacheCoreSetting.getCoreSetting(RC_JASPERREPORTS_ROOT_HTTP) + "?reportdir=NKP_OBS_PREDLOHA&reportFileName=obsahovaPredloha&CAN=" + can + "&action=Print+report\"</SCRIPT>");
        return rwm;
    }
