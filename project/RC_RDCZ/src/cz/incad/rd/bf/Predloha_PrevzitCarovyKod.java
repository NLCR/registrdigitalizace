 ...
 * @author martin
 */
public class Predloha_PrevzitCarovyKod extends BussinessFunctionMother {
     private static final String LOCAL_SECURITY = "data115";
     public static final String f_odKoho = "odKoho";
    ...
    public WizardMessage panelLeave(String panelName) throws ValidationException, WizardException {
        ...
        if ("panel01".equals(panelName)) {
            this.komu = (String) getWizardContextClient().getWizardRecord().getSimpleField(Predloha_PrevzitCarovyKod.f_odKoho).getValue();
            this.varka = (String) getWizardContextClient().getWizardRecord().getSimpleField(Predloha_PrevzitCarovyKod.f_varka).getValue();

        } else if ("panel02".equals(panelName)) {
            //otestujeme zdali je CK null a když ne tak přidáme další panel.
            if (getWizardContextClient().getWizardRecord().getSimpleField(Predloha_PrevzitCarovyKod.f_carKod).getValue() != null) {
                wPanels.add(wPanels.size() -1, wPanels.get(1));
                getWizardContextClient().getWizardController().rebuildWizard();

                //Získáme čárkód
                carKod = (String) getWizardContextClient().getWizardRecord().getSimpleField(Predloha_PrevzitCarovyKod.f_carKod).getValue();
                countZadaneCarKody++;
                ...
                getWizardContextClient().getWizardRecord().getSimpleField(Predloha_PrevzitCarovyKod.f_carKod).setValue(null);
    ...
    public WizardMessage runBusinessMethod() throws ValidationException, WizardException, ApplicationErrorException {
        ...
        for (int i = 0; i < listPredlohyPUK.size(); i++) {
            ...
            try {
                ...
                Predloha_PrevzitCarovyKod.doOnePredaniPrevzeti(recPredloha, "prevzeti", this.komu, this.varka, getWizardContextClient());
            } catch (Exception ex) {
